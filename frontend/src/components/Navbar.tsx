...
import {Alert, Avatar, Badge, Button, Divider, PageHeader, Typography} from "antd";
...
export const Navbar: FC<Props> = (props) => {
    ...
    const {players, turnStartDate, meIsCurrent} = usePlayers()
                ...
                {players?.map(e => {
                    return <Avatar key={e.id} shape={e.current ? 'square' : 'circle'} size={'large'}
                                   style={{
                                       background: e.color,
                                       borderColor: e.color,
                                       border: e.bankrupt ? `4px dashed ${red[8]}` : `1px solid ${e.color}`
                                   }}
                                   src={<AvatarIcon value={e.username} size={32}/>}/>;
                })}
            </Avatar.Group>
        </div>}
                    subTitle={<>{meIsCurrent && gameStarted && <Alert message="Your turn" type="success"/>}</>}
                    extra={[
